	...
	internal class Program
	{
		...
		private async Task SendFakeData(CancellationToken token)
		{
			...
			while (!token.IsCancellationRequested)
			{
				...
				await _connection.InvokeAsync(nameof(PumpControlHub.UpdateStatus), pump, token);
				...
				await _connection.InvokeAsync(nameof(PumpControlHub.UpdateStatus), tank, token);
