	...
	public class TodoItemSpec : TestContext
	{
		...
		public void clicking_DeleteButton_calls_OnDelete_callback()
		{
			...
			Assert.Raises<MouseEventArgs>(
				handler => SUT.SetParametersAndRender((nameof(TodoListItem.OnDelete), AssertEx.AsCallback(handler))),
		...
		public void toggling_Checked_triggers_OnUpdate()
		{
			Assert.Raises<EventArgs>(
				handler => SUT.SetParametersAndRender((nameof(TodoListItem.OnUpdate), AssertEx.AsCallback(handler))),
				_ => {},
				() => SUT.Find("p").Click());
		}
